...
1. **Shot-count (`BTC_MIN_SHOTS = 3`):** Offer BTC only when the payment unlocks **≥ 3 photos** (bundle / multi-shot upsell; `shot_ids.length` on create). Single-shot and 2-shot gates never show BTC. Tip / `createInvoice` reject: “Bitcoin is available when unlocking 3 or more photos” (`BTC_SHOT_MIN_BUYER_TIP` / `btcBelowShotMinCreateError`).
